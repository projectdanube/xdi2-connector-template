package xdi2.connector.template;
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if ("Request Access Token!".equals(request.getParameter("submit"))) {
			...
			try {
				...
				this.getTemplateApi().startOAuth(request, response, userXri);
				return;
			} catch (Exception ex) {
		...
		if (request.getParameter("code") != null) {
			...
			try {
				...
				this.getTemplateApi().checkState(request, userXri);

				String accessToken = this.getTemplateApi().exchangeCodeForAccessToken(request);
